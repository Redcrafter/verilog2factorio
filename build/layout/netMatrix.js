...
function overlaps(list, inter) {
    for (let i = 0; i < list.length; i++) {
        const el = list[i];
        if (inter.min <= el.max && el.min <= inter.max)
            return true;
    }
    return false;
}
export function createMatrixLayout(combs, ports) {
    ...
    function stringPoles(el, x) {
        ...
        while (j < points.length) {
            ...
            grid[x][nextY] = p;
            if (last)
                connect(el.net.color == "red" ? 1 /* Red */ : 2 /* Green */, last.output, p.output);
            last = p;
            lastY = nextY;
        }
    }
    ...
    for (let i = 0; i < combs.length; i++) {
        ...
        let inCol = (item.input.red ? 1 /* Red */ : 0) | (item.input.green ? 2 /* Green */ : 0);
        ...
        let outCol = (item.output.red ? 1 /* Red */ : 0) | (item.output.green ? 2 /* Green */ : 0);
        for (let j = 1; j <= needed; j++) {
            let b = new Constant();
            b.isOn = false;
            newCombs.push(b);
        ...
        if (item.input.red) {
            ...
            connect(1 /* Red */, p.output, inPoles[Math.floor(item.input.red.id / 8)].input);
        }
        if (item.input.green) {
            let p = grid[item.input.green.id][i];
            connect(2 /* Green */, p.output, inPoles[Math.floor(item.input.green.id / 8)].input);
        }
        if (item.output.red) {
            let p = grid[item.output.red.id][i];
            connect(1 /* Red */, p.output, outPoles[Math.floor(item.output.red.id / 8)].output);
        }
        if (item.output.green) {
            let p = grid[item.output.green.id][i];
            connect(2 /* Green */, p.output, outPoles[Math.floor(item.output.green.id / 8)].output);
        }
    }
