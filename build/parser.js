...
export function buildGraph(mod) {
    ...
    function findMergeList(bits) {
        ...
        while (offset < bits.length) {
            ...
            let sect = matchSection(bits.slice(offset));
            if (!sect)
                throw new Error("Unreachable"); // should be covered by knownWires
            offset += sect.count;
            sub.push(sect);
        }
        return sub;
    }
    function getInputNode(bits) {
        let allConst = true;
        let value = 0;
        ...
        let node = new MergeNode(sub, bits);
        nodes.push(node);
        return node;
    }
